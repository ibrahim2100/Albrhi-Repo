...
static void SCIBuildTables(void) {
    _enTable = @{
        ...
        @"watch_updates_footer": @"With this on, the Watch app stops going looking for watchOS updates, so none is offered or installed through this iPhone. It is a hold, not a filter — it does not yet distinguish one watchOS version from another, and it needs the Watch app reopened to take effect.",
    ...
    _arTable = @{
        ...
        @"watch_updates_footer": @"بتشغيله يتوقّف تطبيق Watch عن البحث عن تحديثات watchOS، فلا يُعرَض تحديثٌ ولا يُثبَّت عبر هذا الآيفون. وهو إيقافٌ لا ترشيح — لا يفرّق بعدُ بين نسخة وأخرى، ويحتاج إعادة فتح تطبيق Watch ليأخذ مفعوله.",
