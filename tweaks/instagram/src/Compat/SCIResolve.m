...
static void SCIBuildIndex(void) {
    ...
    dispatch_once(&once, ^{
        ...
        sciAnswers = [NSMutableDictionary dictionary];
...
Class SCIResolveClass(NSString *name) {
    ...
    // The plain name first: on 410 every one of these resolves here and the index is never
    // consulted, which is also what keeps the older build's behaviour identical.
