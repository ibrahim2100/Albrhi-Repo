import Foundation
import ObjectiveC
// Upstream imports EeveeSpotifyC here for two C helpers. This port declares those same two
// functions, under the same names, in its own bridging header -- so the import is unnecessary and
// its module does not exist here. **The only edit made to any ported file**, noted rather than
// left to be discovered by a diff.
