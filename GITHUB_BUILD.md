...
1. Unzip `Albrhi-v3.0.4-source.zip` on your PC — you'll get a `SCInsta-dev` folder.
...
- Download **`com.albrhi.tweak_3.0.4+rootless.deb`** (it comes zipped — unzip to get the `.deb`).
...
```
scp com.albrhi.tweak_3.0.4+rootless.deb mobile@<iphone-ip>:/tmp/
ssh mobile@<iphone-ip> "sudo dpkg -i /tmp/com.albrhi.tweak_3.0.4+rootless.deb && sudo sbreload"
```
