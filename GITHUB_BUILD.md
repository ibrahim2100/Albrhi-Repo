...
1. Unzip `Albrhi-source.zip` on your PC — you'll get a `SCInsta-dev` folder.
...
- Download **`com.albrhi.tweak_*+rootless.deb`** (it comes zipped — unzip to get the `.deb`).
...
```
scp com.albrhi.tweak_*+rootless.deb mobile@<iphone-ip>:/tmp/
ssh mobile@<iphone-ip> "sudo dpkg -i /tmp/com.albrhi.tweak_*+rootless.deb && sudo sbreload"
```
